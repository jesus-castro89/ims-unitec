...
class ClientModel extends AbstractModel
{
    ...
    public function getForm(ContainerInterface $container, ServerRequestInterface $request, ResponseInterface $response, array $args)
    {
        ...
        $object = $this->findOneByPrimaryKey("ClientId");
        $form = $this->configureForm(ClientForm::class, $object);
        // Procesar Formulario
        ...
        if ($form->isSubmitted() && $form->isValid()) {
            ...
            $resp = new RedirectResponse($routeParser->urlFor('clientDashboard'));
            $resp->prepare($req);
            return $resp->send();
        }
        // Renderizar formulario
        try {
            return $container->get('view')->render($response, 'clientForm.twig', [
    ...
    public function tableLoader(ServerRequestInterface $request, ResponseInterface $response, array $args): ResponseInterface
    {
        ...
        $next = $router->fullUrlFor($request->getUri(), "clientTable") . "/$limit/" . ($offset + $limit);
        $previous = $offset == 0 ? null : $router->fullUrlFor($request->getUri(), "clientTable") . "/$limit/" . ($offset - $limit);
        if (!is_null($search)) {
            $count = $this->query->filterByClientName("%" . $search . "%", Criteria::LIKE)->count();
            $data = $this->query->filterByClientName("%" . $search . "%", Criteria::LIKE)->limit($limit)->offset($offset);
        } else {
